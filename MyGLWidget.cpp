...
void MyGLWidget::initializeGL()
{
    ...
    // texture
    m_tex_brickwall = new QOpenGLTexture(QImage(":/image/brickwall.jpg"));
    m_tex_brickwall->bind(TEX_CH_BRICK);

    m_tex_normal = new QOpenGLTexture(QImage(":/image/brickwall_normal.jpg"));
...
{
    if(!m_camera){
        m_camera = new Camera(QVector3D(0.0f, 0.0f, 3.0f), QVector3D(0.0f, 1.0f, 0.0f), radians(0.0f), radians(180.0f));
    }

    static float model_angle_x = -30;
